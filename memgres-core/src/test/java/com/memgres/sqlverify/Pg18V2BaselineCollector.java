...
/**
 * Collects baseline results by running the pg18_sample_sql_2 verification files
 * against a real PostgreSQL 18 instance.
 *
 * Usage:
 *   mvn -pl memgres-core exec:java \
 *     -Dexec.mainClass="com.memgres.sqlverify.Pg18V2BaselineCollector" \
 *     -Dexec.classpathScope=test
 *
 * Requires PostgreSQL on localhost:5432, database 'memgrestest',
 * user 'memgres', password 'memgres'.
 *
 * Outputs: memgres-core/src/test/resources/sql-verify-pg18-v2/pg-baseline.txt
 */
public class Pg18V2BaselineCollector {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("=== PG18 V2 Baseline Collector ===\n");
    ...
    private static void cleanDatabase() {
        try (Connection conn = DriverManager.getConnection(PG_URL, PG_USER, PG_PASS)) {
            ...
            try (Statement s = conn.createStatement()) {
                ...
                // Drop policies (RLS tests create these)
                dropByQuery(s, "SELECT 'DROP POLICY IF EXISTS ' || quote_ident(polname) || ' ON ' || quote_ident(schemaname) || '.' || quote_ident(tablename) FROM pg_policies WHERE schemaname = 'public'");
