...
/**
 * Collects baseline results by running SQL verification files against a real PostgreSQL instance.
 *
 * Usage:
 *   mvn -pl memgres-core exec:java \
 *     -Dexec.mainClass="com.memgres.sqlverify.PgBaselineCollector" \
 *     -Dexec.classpathScope=test
 *
 * Requires PostgreSQL on localhost:5432, database 'memgrestest',
 * user 'memgres', password 'memgres'.
 *
 * Outputs: memgres-core/src/test/resources/sql-verify/pg-baseline.txt
 */
public class PgBaselineCollector {
    ...
    public static void main(String[] args) throws Exception {
        System.out.println("=== PG Baseline Collector ===\n");
        ...
        try (Connection conn = DriverManager.getConnection(PG_URL, PG_USER, PG_PASS)) {
            ...
            Path outputPath = SqlVerifyHarness.findResourceDir().resolve("pg-baseline.txt");
    ...
    private static void cleanDatabase() {
        ...
        try (Connection conn = DriverManager.getConnection(url, PG_USER, PG_PASS)) {
