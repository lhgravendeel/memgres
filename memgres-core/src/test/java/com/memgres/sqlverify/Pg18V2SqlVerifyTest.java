...
/**
 * Runs the pg18_sample_sql_2 verification files against memgres and compares
 * results to the PG 18 baseline captured by Pg18V2BaselineCollector.
 *
 * The PG baseline must be collected first by running Pg18V2BaselineCollector.
 * If the baseline file is missing, this test will fail with a clear message.
 */
class Pg18V2SqlVerifyTest {
    ...
    void verifySqlCompatibility() throws Exception {
        ...
        // Run against memgres
        System.out.println("Running PG18 V2 SQL verification against memgres...\n");
        ...
        if (!diffs.isEmpty()) {
            ...
            report.append("PG18 V2 SQL verification found ").append(diffs.size()).append(" differences:\n\n");
