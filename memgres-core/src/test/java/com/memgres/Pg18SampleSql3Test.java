...
/**
 * Test runner for the pg18_sample_sql_3 suite.
 *
...
class Pg18SampleSql3Test {
    ...
    void runAllSqlFiles() throws Exception {
        ...
        for (Path sqlFile : sqlFiles) {
            ...
            System.out.printf("[%s] %-50s  pass=%d  fail=%d%n", status, fileName, filePass, fileFail);
        }
            ...
            if (line.equals("-- begin-expected")) {
                ...
                while (i < lines.size()) {
                    ...
                    if (el.startsWith("-- columns:")) {
                        columns = Arrays.stream(el.substring("-- columns:".length()).trim().split("\\|"))
    ...
    private static Path findSuiteDir() {
        try {
            java.net.URL url = Pg18SampleSql3Test.class.getClassLoader().getResource("pg18-sql-3");
            if (url != null) return Path.of(url.toURI());
        } catch (Exception ignored) {}
        throw new RuntimeException("Cannot find pg18-sql-3 resource directory");
    }
}
