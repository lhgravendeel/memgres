...
class CustomOperatorsTest {
     ...
     *
     * ~++ is a valid operator name (starts with ~), so schema-qualified usage should succeed.
     * The operator was created in setUp with function op_int_add_10: a + b + 10.
     */
    @Test
    @Order(1)
    void testStmt16_qualifiedTildePlusPlusShouldSucceed() throws SQLException {
        String result = query1("SELECT 2 OPERATOR(op_test.~++) 3 AS result");
        assertEquals("15", result,
                "OPERATOR(op_test.~++) should return 2 + 3 + 10 = 15");
    }
     ...
     *
     * ~++ is a valid operator name. The text overload was created in setUp
     * with function op_text_concat_bang: a || '!' || b.
     */
    @Test
    @Order(3)
    void testStmt21_tildePlusPlusTextShouldSucceed() throws SQLException {
        String result = query1("SELECT 'hello'::text ~++ 'world'::text AS result");
        assertEquals("hello!world", result,
                "text ~++ text should return 'hello!world'");
    }

    /**
     * Stmt 60: CREATE FUNCTION with OPERATOR(op_test.~++) in PL/pgSQL body
     *
     * ~++ is a valid operator name, so the function creation should succeed.
     */
    @Test
    @Order(4)
    void testStmt60_createFunctionWithQualifiedTildePlusPlusShouldSucceed() throws SQLException {
     ...
     *
     * The function was created successfully in stmt 60 using OPERATOR(op_test.~++),
     * which maps to op_int_add_10: a + b + 10.
     */
    @Test
    @Order(5)
    void testStmt61_callQualifiedPlpgsqlFunctionShouldSucceed() throws SQLException {
        String result = query1("SELECT op_qualified_in_plpgsql(2, 3) AS result");
        assertEquals("15", result,
                "op_qualified_in_plpgsql(2, 3) should return 2 + 3 + 10 = 15");
    }
