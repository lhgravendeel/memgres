...
-- finding: Root cause 4: privilege checks read only TABLE-keyed grants
-- area: DDL objects: indexes, sequences, types, functions, triggers, rules, views, partitioning and the catalogs that describe them
-- title: Root cause 4: privilege checks read only TABLE-keyed grants
-- begin-expected
-- ok: 0
-- end-expected
...
-- begin-expected-error
-- sqlstate: 42P01
-- message-like: relation "zz_vf_pt2" does not exist
-- end-expected-error
GRANT SELECT ON zz_vf_pt2 TO zz_vf_r2;
-- begin-expected-error
-- sqlstate: 42704
-- message-like: role "zz_vf_r2" does not exist
-- end-expected-error
...
SELECT column_name, privilege_type FROM information_schema.column_privileges
 WHERE table_name='zz_vf_pt' AND grantee='zz_vf_r' ORDER BY 1,2;
-- begin-expected-error
-- sqlstate: 42P01
-- message-like: relation "zz_vf_pt2" does not exist
-- end-expected-error
GRANT SELECT ON zz_vf_pt, zz_vf_pt2 TO zz_vf_r2;
-- begin-expected
-- ok: 0
-- end-expected
GRANT SELECT ON zz_vf_pt TO GROUP zz_vf_r2;
-- begin-expected
-- ok: 0
-- end-expected
GRANT zz_vf_r2 TO zz_vf_r WITH ADMIN TRUE;
