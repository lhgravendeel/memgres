...
-- finding: Root cause 4: WITH is executed without the analysis PostgreSQL performs first
-- area: Joins, CTEs, subqueries — and the DDL that came with them
-- title: Root cause 4: WITH is executed without the analysis PostgreSQL performs first
...
-- end-expected-error
WITH RECURSIVE g(id, link) AS (SELECT id, link FROM zz_dag WHERE id = 1
  UNION ALL SELECT gg.id, gg.link FROM zz_dag gg, g WHERE gg.id = g.link)
CYCLE id, id SET c USING p SELECT count(*) FROM g;
-- begin-expected-error
-- sqlstate: 42601
-- message-like: syntax error at or near ".."
-- end-expected-error
WITH RECURSIVE g(id, link) AS (...) CYCLE id SET id USING p SELECT count(*) FROM g;
-- begin-expected-error
-- sqlstate: 42P07
