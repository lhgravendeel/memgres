...
-- finding: MERGE runs on a code path of its own that skips every check the other DML paths make
-- area: DML, MERGE, partitioning, rules and the COPY/extended-protocol surface
-- title: MERGE runs on a code path of its own that skips every check the other DML paths make
-- begin-expected
-- ok: 0
-- end-expected
...
-- end-expected-error
MERGE INTO zz_vf_st t USING (VALUES (1),(2)) AS v(id) ON t.id = v.id
