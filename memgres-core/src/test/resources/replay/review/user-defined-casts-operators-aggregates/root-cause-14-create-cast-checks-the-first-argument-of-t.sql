...
-- finding: Root cause 14: CREATE CAST checks the first argument of the cast function and the user-defined cast list, and nothing else
-- area: User-defined casts, operators, aggregates, collations and extensions
-- title: Root cause 14: CREATE CAST checks the first argument of the cast function and the user-defined cast list, and nothing else
-- begin-expected
-- ok: 0
-- end-expected
...
-- end-expected-error
CREATE FUNCTION zz_f4(zz_v, text)     RETURNS int LANGUAGE sql IMMUTABLE AS $$ SELECT 1 $$;
-- begin-expected-error
-- sqlstate: 42704
-- message-like: type zz_v does not exist
-- end-expected-error
CREATE FUNCTION zz_f5(zz_v, int, int) RETURNS int LANGUAGE sql IMMUTABLE AS $$ SELECT 1 $$;
-- begin-expected-error
-- sqlstate: 42704
-- message-like: type "zz_v" does not exist
-- end-expected-error
