...
-- finding: Root cause 5: CREATE COLLATION records a name and almost nothing else
-- area: User-defined casts, operators, aggregates, collations and extensions
-- title: Root cause 5: CREATE COLLATION records a name and almost nothing else
-- begin-expected
-- ok: 0
-- end-expected
CREATE SCHEMA zz_cs;
...
-- begin-expected
-- columns: nspname:name
-- row: zz_cs
-- rowcount: 1
-- end-expected
SELECT n.nspname FROM pg_collation c JOIN pg_namespace n ON n.oid=c.collnamespace WHERE c.collname='zz_sc';
-- begin-expected
-- ok: 0
-- end-expected
...
-- row: CREATE INDEX zz_ci ON public.zz_ct USING btree (s COLLATE zz_c)
-- rowcount: 1
-- end-expected
