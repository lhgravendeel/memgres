...
-- finding: Root cause 3: a role OID can never be turned back into a role name
-- area: System catalogs, information_schema and security
-- title: Root cause 3: a role OID can never be turned back into a role name
-- begin-expected
-- ok: 0
-- end-expected
...
-- rowcount: 1
-- end-expected
SELECT has_table_privilege(999999999::oid, 'SELECT')::text,
       has_table_privilege(999999999::oid, 'zz_ht', 'SELECT')::text,
       has_table_privilege('memgres', 999999999::oid, 'SELECT')::text;
-- begin-expected-error
-- sqlstate: 42P07
