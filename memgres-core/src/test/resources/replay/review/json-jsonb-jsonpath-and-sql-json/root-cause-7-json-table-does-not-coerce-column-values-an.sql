...
-- finding: Root cause 7: JSON_TABLE does not coerce column values and drops sibling NESTED paths
-- area: JSON, JSONB, jsonpath and SQL/JSON
-- title: Root cause 7: JSON_TABLE does not coerce column values and drops sibling NESTED paths
-- begin-expected
-- columns: a:int4
-- row: NULL
-- rowcount: 1
...
-- rowcount: 3
-- end-expected
SELECT * FROM JSON_TABLE(jsonb '{"a":[1,2],"b":[3]}', '$'
  COLUMNS (NESTED '$.a[*]' COLUMNS (av int PATH '$'),
           NESTED '$.b[*]' COLUMNS (bv int PATH '$'))) ORDER BY 1,2;
-- begin-expected
-- columns: a:int4
-- row: 1
-- row: 2
