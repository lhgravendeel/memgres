...
-- finding: Root cause 1: the jsonpath engine is a text walker, not a parser
-- area: JSON, JSONB, jsonpath and SQL/JSON
-- title: Root cause 1: the jsonpath engine is a text walker, not a parser
-- begin-expected
...
-- sqlstate: 42704
-- message-like: could not find jsonpath variable "nope"
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '$[*] ? (@ == $nope)', '{}');
-- begin-expected-error
-- sqlstate: 42704
-- message-like: could not find jsonpath variable "__last"
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '$[*] ? (@ == $__last)', '{}');
-- begin-expected-error
-- sqlstate: 22P02
-- message-like: invalid input syntax for type json
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '$.a', 'notjson');
-- begin-expected-error
-- sqlstate: 22023
-- message-like: "vars" argument is not an object
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '$.a', '[1]');
-- begin-expected
-- columns: jsonb_path_query:jsonb
...
-- begin-expected-error
-- sqlstate: 42601
-- message-like: syntax error at end of jsonpath input
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '$.');
-- begin-expected-error
-- sqlstate: 22P02
-- message-like: invalid input syntax for type jsonpath: ""
-- end-expected-error
SELECT jsonb_path_query('{"a":1}', '');
-- begin-expected
-- columns: jsonb_path_query:jsonb
