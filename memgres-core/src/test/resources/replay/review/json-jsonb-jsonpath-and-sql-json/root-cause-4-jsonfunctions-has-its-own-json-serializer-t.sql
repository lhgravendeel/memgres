...
-- finding: Root cause 4: JsonFunctions has its own JSON serializer that escapes only `\` and `"`
-- area: JSON, JSONB, jsonpath and SQL/JSON
-- title: Root cause 4: JsonFunctions has its own JSON serializer that escapes only `\` and `"`
-- begin-expected
