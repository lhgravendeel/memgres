...
-- finding: Root cause 13: relation locks exist only on the DML write path
-- area: PREPARE, EXECUTE, DO and CALL
-- title: Root cause 13: relation locks exist only on the DML write path
-- session B: BEGIN; SELECT count(*) FROM zz_vf2_dl;
-- session A, with SET lock_timeout='2000ms':
