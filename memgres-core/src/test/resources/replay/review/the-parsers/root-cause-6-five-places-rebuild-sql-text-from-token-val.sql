...
-- finding: Root cause 6: five places rebuild SQL text from Token.value(), which no longer carries the quoting
-- area: The parsers
-- title: Root cause 6: five places rebuild SQL text from Token.value(), which no longer carries the quoting
-- begin-expected
-- ok: 0
-- end-expected
...
-- end-expected-error
CREATE TRIGGER zz_vf2_tg BEFORE INSERT ON zz_vf2_tw FOR EACH ROW WHEN (NEW."a b" > 0)
  EXECUTE FUNCTION zz_vf2_twf();
