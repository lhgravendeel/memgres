...
-- finding: 224
-- title: catalog columns are declared with the nearest ordinary SQL type instead of the catalog's real type (INTEGER for oid, TEXT for name and for the array-valued colu
-- begin-expected
...
-- rowcount: 1
-- end-expected
SELECT pg_typeof(oid), pg_typeof(extname), pg_typeof(extowner), pg_typeof(extconfig), pg_typeof(extcondition) FROM pg_extension WHERE extname='plpgsql';
-- begin-expected
-- columns: pg_typeof:regtype | pg_typeof:regtype | pg_typeof:regtype
-- row: oid | name | oid
-- rowcount: 1
