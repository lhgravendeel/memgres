...
-- finding: 208
-- title: Recorded DDL options are never made to mean anything: CREATE CAST writes a pg_cast row no cast path reads, CREATE OPERATOR CLASS keeps a name and drops its OPER
-- begin-expected
-- ok: 0
-- end-expected
...
-- message-like: relation "zz_t" does not exist
-- end-expected-error
SELECT c.relname::text, i.indisclustered FROM pg_index i JOIN pg_class c ON c.oid=i.indexrelid WHERE i.indrelid='zz_t'::regclass ORDER BY 1;
-- begin-expected-error
-- sqlstate: 42P01
-- message-like: relation "zz_t" does not exist
-- end-expected-error
