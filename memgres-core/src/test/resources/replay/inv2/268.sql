...
-- finding: 268
-- title: Privileges are stored as a flat set of "PRIV:OBJTYPE:name" strings, and both the checker and the catalog views read that string literally. GRANT ALL writes one 
-- begin-expected
-- ok: 0
-- end-expected
...
-- rowcount: 8
-- end-expected
SELECT table_name, privilege_type, is_grantable FROM information_schema.table_privileges WHERE grantee='zz_yr' ORDER BY table_name, privilege_type;
-- begin-expected-error
-- sqlstate: 42P07
-- message-like: relation "zz_y1" already exists
-- end-expected-error
...
-- rowcount: 1
-- end-expected
SELECT attname, attacl::text FROM pg_attribute WHERE attrelid='zz_g1'::regclass AND attacl IS NOT NULL ORDER BY attname;
-- begin-expected
-- columns: column_name:name | privilege_type:varchar
-- row: b | SELECT
-- rowcount: 1
-- end-expected
SELECT column_name, privilege_type FROM information_schema.column_privileges WHERE table_name='zz_g1' AND grantee='zz_r2' ORDER BY column_name;
-- begin-expected
-- ok: 0
-- end-expected
CREATE ROLE zz_hr;
-- begin-expected
-- columns: c:text | i:text
-- row: true | true
-- rowcount: 1
-- end-expected
SELECT has_table_privilege('zz_hr','pg_class','SELECT')::text AS c, has_table_privilege('zz_hr','information_schema.tables','SELECT')::text AS i;
-- begin-expected
...
-- begin-expected
-- columns: makeaclitem:text
-- row: memgres=r/memgres
-- rowcount: 1
-- end-expected
SELECT makeaclitem((SELECT oid FROM pg_roles WHERE rolname=current_user),(SELECT oid FROM pg_roles WHERE rolname=current_user),'SELECT',false)::text;
