...
-- finding: 331
-- title: A relation whose target is not a plain Table falls out of the catalog builders: the index pg_class loop `continue`s when schema.getTable() returns null, relispa
...
-- rowcount: 1
-- end-expected
SELECT i.relname, i.relispartition FROM pg_index x JOIN pg_class i ON i.oid = x.indexrelid JOIN pg_class t ON t.oid = x.indrelid WHERE t.relname = 'zz_vf2_pta';
-- begin-expected
