...
-- finding: 70
-- title: Java's toString()/equals() stands in for the type's output function or its = operator when building identity keys and error text: DISTINCT ON keys, the CREATE U
-- begin-expected
-- ok: 0
-- end-expected
...
DECLARE x int := 1;
BEGIN
  CASE x WHEN 1.0 THEN RETURN 'one'; ELSE RETURN 'other'; END CASE;
...
-- ok: 0
-- end-expected
DROP FUNCTION zz_vf_case();
-- begin-expected
-- ok: 0
-- end-expected
CREATE FUNCTION zz_vf_assertarr() RETURNS void AS $$
BEGIN ASSERT false, ARRAY[1,2]; END $$ LANGUAGE plpgsql;
-- begin-expected-error
-- sqlstate: P0004
-- message-like: {1,2}
-- end-expected-error
SELECT zz_vf_assertarr();
-- begin-expected
-- ok: 0
-- end-expected
DROP FUNCTION zz_vf_assertarr();
-- begin-expected
-- ok: 0
-- end-expected
