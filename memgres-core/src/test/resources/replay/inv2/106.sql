...
-- finding: 106
-- title: An aggregate is bound to the query it is syntactically written in rather than to the query level its argument variables come from. containsAggregate returns fal
-- begin-expected
-- ok: 0
-- end-expected
...
-- rowcount: 1
-- end-expected
SELECT a.g FROM zz_vf_a a GROUP BY a.g HAVING (SELECT count(*) FROM zz_vf_b b WHERE b.id = min(a.id)) > 0 ORDER BY a.g;
