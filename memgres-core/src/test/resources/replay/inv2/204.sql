...
-- finding: 204
-- title: A user-defined aggregate is looked up by bare name and evaluated positionally, mirroring the built-in aggregate defect the August report named at SelectExecutor
-- begin-expected
-- ok: 0
-- end-expected
...
-- ok: 0
-- end-expected
CREATE FUNCTION zz_ud_cat(text, text) RETURNS text LANGUAGE sql IMMUTABLE AS $$ SELECT COALESCE($1,'') || COALESCE($2,'') $$;
-- begin-expected
-- ok: 0
-- end-expected
CREATE AGGREGATE zz_ud_agg (text) (SFUNC = zz_ud_cat, STYPE = text, INITCOND = '');
-- begin-expected
-- columns: zz_ud_agg:text
-- row: rqp
-- rowcount: 1
-- end-expected
