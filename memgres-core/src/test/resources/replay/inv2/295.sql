...
-- finding: 295
-- title: A default-privilege entry has no real owner: the grantor defaults to the session user rather than the current role, and removeDefaultAcl never compares the gran
-- begin-expected
-- ok: 0
-- end-expected
CREATE ROLE zz_own;
...
-- rowcount: 1
-- end-expected
SELECT count(*) FROM pg_default_acl d JOIN pg_namespace n ON n.oid=d.defaclnamespace WHERE n.nspname='zz_xs';
