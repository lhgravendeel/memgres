...
-- finding: 110
-- title: An aggregate call is dispatched by name alone. There is no signature resolution, so the argument count, the argument types, the * form and the direct arguments 
...
-- end-expected-error
SELECT sum(v) OVER (ORDER BY v RANGE BETWEEN '1'::text PRECEDING AND CURRENT ROW) FROM (VALUES (10),(20),(30)) t(v) ORDER BY 1;
-- begin-expected
-- ok: 0
-- end-expected
...
-- begin-expected-error
-- sqlstate: 42883
-- message-like: function max() does not exist
-- end-expected-error
SELECT max(*) FROM (VALUES (1)) t(v);
-- begin-expected-error
-- sqlstate: 42809
