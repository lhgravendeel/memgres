...
-- A collation the real server does not have is a hazard, not clutter: a tool
-- that reads pg_collation and offers it writes SQL that fails there.
-- begin-expected
-- columns: n
-- row: 0
-- end-expected
SELECT count(*) AS n FROM pg_collation
WHERE collname IN ('C.UTF-8', 'C.utf8', 'en_US.UTF-8', 'en_US.utf8');
