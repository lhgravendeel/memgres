...
final class StoredExprNames {
    ...
    private void requireFunctionExists(FunctionCallExpr call) {
        ...
        // A name carrying a schema of its own is that schema's to answer for, and a schema that
        // does exist answers for what is in it.
        ...
        boolean readAsCast = call.args() != null && call.args().size() == 1
        ...
        if (BuiltinFunctionNames.isCallable(bare)) {
            if (!readAsCast) requireArgumentTypesReach(call, written, bare);
            return;
        }
